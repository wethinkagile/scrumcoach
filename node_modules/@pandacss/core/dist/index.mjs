...
import { compact as compact2, flatten as flatten2, isBoolean, isString as isString3, memo as memo6, patternFns } from "@pandacss/shared";
...
var Context = class {
  ...
  get prefix() {
    return {
      tokens: isString3(this.config.prefix) ? this.config.prefix : this.config.prefix?.cssVar,
      className: isString3(this.config.prefix) ? this.config.prefix : this.config.prefix?.className
    };
  }
  ...
  setupCompositions = (theme) => {
    ...
    const compositions = compact2({ textStyle: textStyles, layerStyle: layerStyles });
    const stylesheetCtx = {
